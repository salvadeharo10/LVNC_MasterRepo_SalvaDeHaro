...
from data.datasetWithYOLO import LVNCDataset, get_classes_proportion
...
########################################################################################
    ...
    for i in folds:
        ...
        train_ds = LVNCDataset(os.path.join(cfg["data_folder"], "new_raw_dataset"), preproc_folder, train_split, yolo_model, transform)
        val_ds = LVNCDataset(os.path.join(cfg["data_folder"], "new_raw_dataset"), preproc_folder, val_split, yolo_model)
...
########### Initialize the trainer and fit ########################################################
        ...
        trainer = Trainer(callbacks=callbacks, logger=loggers, default_root_dir=fold_dir, **cfg["trainer"])
        
        trainer.fit(network, train_dataloader = train_dl, val_dataloaders =val_dl)
...
###################################################################################################
        ...
        test_ds = LVNCDataset(os.path.join(cfg["data_folder"], "new_raw_dataset"), preproc_folder, test_split, yolo_model)
